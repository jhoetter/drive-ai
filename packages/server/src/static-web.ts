...
function resolveWebDist(): string | null {
  const explicit = (process.env["DRIVEAI_WEB_DIST"] ?? "").trim();
  const candidates = [
    explicit,
    "/app/web",
...
export async function registerStaticWeb(app: FastifyInstance): Promise<void> {
  ...
  if (!dist) {
    app.log.info("DriveAI web dist not found; serving API-only backend");
    return;
  }
  ...
  app.setNotFoundHandler((request, reply) => {
    const path = new URL(request.url, "http://driveai.local").pathname;
